...
/*
        ...
        ImGui.SetCursorPosX(ImGui.GetContentRegionAvail().X / 2 - editorSize.X / 2);
        if (ImGui.Button("Load Image", editorSize with { Y = ImGui.GetTextLineHeightWithSpacing() * 2 })) {
            controlFlags |= WindowControlFlags.PreventClose;
            MainWindow!.AllowAutoClose = false;
            fileDialogManager.Reset();
            fileDialogManager.OpenFileDialog("Select Image...", $"Image Files ({string.Join(' ', Common.SupportedImageFileTypes)}){{{string.Join(',', Common.SupportedImageFileTypes.Select(t => $".{t}"))}}}", imageProvider.LoadFile, 1, startPath: PluginConfig.ImageFilePickerLastPath.OrDefault(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures)));
        }
